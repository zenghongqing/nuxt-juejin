<template>
  <div>
    <div class="container-view welcome-view">
      <div class="feed welcome_feed">
        <nav class="category-nav">
          <h5>热门文章</h5>
          <div class="more" @click="handleMore">查看更多</div>
          <ul class="nav-list">
            <li class="nav-item" v-for="(category, index) in categories" :key="index">
              <nuxt-link :to="category.router">{{category.title}}</nuxt-link>
            </li>
          </ul>
        </nav>
        <button class="to-top-btn" title="回到顶部" @click="goTop" v-show="isShowTopBtn">
          <!--<v-icon name="android-arrow-dropup" size="16"></v-icon>-->
          <!--<i class="el-icon-caret-top"></i>-->
          <v-icon name="android-arrow-dropup" size="24"></v-icon>
        </button>
        <nuxt/>
      </div>
    </div>
  </div>
</template>

<script>
import VIcon from "../components/icon";
export default {
  components: {
    VIcon
  },
  data () {
    return {
      categories: [
        { router: { name: 'index' }, title: '推荐' },
        { router: { path: '/welcome/frontend' }, title: '前端' },
        { router: { path: '/welcome/android' }, title: 'Android' },
        { router: { path: '/welcome/backend' }, title: '后端' },
        { router: { path: '/welcome/ai' }, title: '人工智能' },
        { router: { path: '/welcome/ios' }, title: 'iOS' },
        { router: { path: '/welcome/freebie' }, title: '工具资源' },
        { router: { path: '/welcome/article' }, title: '阅读' }
      ],
      isShowTopBtn: false
    }
  },
  head () {
    return {
      title: '首页',
      meta: [
        {hid: 'home custom title', name: 'home', content: 'home custom title description'}
      ]
    }
  },
  mounted () {
    window.addEventListener('scroll', this.showTopBtn)
  },
  methods: {
    // throttle () {
    //   let timer = setTimeout(function () {
    //     clearTimeout(timer)
    //
    //   }, 300)
    // },
    showTopBtn () {
      if (document.documentElement.scrollTop + document.documentElement.clientHeight + 50 > document.documentElement.scrollHeight) {
        this.isShowTopBtn = true
      }
    },
    goTop () {
      this.isShowTopBtn = false
      let scrollTop = document.documentElement.scrollTop
      let speed = Math.floor(document.documentElement.scrollTop / 10)
      while (Math.ceil(document.documentElement.scrollTop) !== 0) {
        scrollTop -= speed
        speed -= 10
        document.documentElement.scrollTop = scrollTop + 'px'
      }

    },
    handleMore () {
      this.$message({
        message: 'sorry, 暂未完成此功能 ~',
        type: 'warning'
      })
    }
  }
}
</script>

<style lang="scss" scoped>
  .category-nav {
    background: #fff;
    padding: 1.5rem 2rem;
    display: flex;
    -webkit-box-pack: justify;
    justify-content: space-between;
    border-bottom: 1px solid #f6f6f6;
    h5 {
      font-size: 16px;
      font-weight: 500;
    }
    .more {
      display: none;
    }
    .nav-list {
      display: flex;
      -webkit-box-align: baseline;
      align-items: baseline;
      li.nav-item {
        &:hover {
          color: #007fff;
        }
        &:not(:last-child) {
          margin-right: 20px;
        }
      }
      .nav-item {
        font-size: 14px;
        color: #90979c;
        padding: 0 !important;
        height: initial !important;
        line-height: initial !important;
        .nuxt-link-exact-active {
          color: #007fff;
        }
      }
    }
  }
  .to-top-btn {
    position: fixed;
    right: 2rem;
    bottom: 3rem;
    z-index: 1000;
    padding: 0;
    width: 3.33rem;
    height: 3.33rem;
    line-height: 1rem;
    color: #909090;
    background: #fff;
    border: 1px solid #f1f1f1;
    border-radius: 50%;
    box-shadow: 0 0 5px rgba(0,0,0,.5);
    cursor: pointer;
    /*display: none;*/
    transition: all 0.2s;
    i {
      /*display: block;*/
      /*width: 10px;*/
      /*height: 10px;*/
      /*position: absolute;*/
      /*left: 50%;*/
      /*top: 50%;*/
      /*border-left: 8px solid transparent;*/
      /*border-right: 8px solid transparent;*/
      /*border-bottom: 12px solid #007fff;*/

    }
  }
  .to-top-btn.show {
    display: block;
  }
  @media screen and (max-width: 600px) {
  .category-nav {
    .more {
      display: block;
    }
    .nav-list {
      display: none;
    }
  }
  }
</style>
